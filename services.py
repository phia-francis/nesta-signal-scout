...
class SheetService:
    ...
    def _get_sheet(self):
        ...
        try:
            ...
            logging.warning("Google Sheets auth error: %s", exc)
            return None
    ...
    def ensure_headers(self, sheet) -> None:
        ...
        try:
            ...
            logging.warning("Header check failed: %s", exc)
    ...
    def get_records(self, include_rejected: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.warning("Sheet read error: %s", exc)
            return []

    def get_existing_urls(self, csv_path: str = "Nesta Signal Vault - Sheet1.csv") -> set[str]:
        try:
    ...
    def upsert_signal(self, signal: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logging.warning("Upsert error: %s", exc)
    ...
    def update_signal_by_url(self, req: UpdateSignalRequest) -> Dict[str, str]:
        ...
        if not match_row:
            new_row = [
                ...
                else (req.score_evocativeness if req.score_evocativeness is not None else 0),
    ...
    def update_local_csv(self, url: str, analysis: str, implication: str, csv_path: str = "Nesta Signal Vault - Sheet1.csv") -> bool:
        ...
        try:
            ...
            logging.warning("CSV read error: %s", exc)
            return False
...
class MockSheetService:
    def get_records(self, include_rejected: bool = False) -> List[Dict[str, Any]]:
        logging.info("MockSheetService: get_records called (include_rejected=%s)", include_rejected)
        return []

    def get_existing_urls(self, csv_path: str = "Nesta Signal Vault - Sheet1.csv") -> set[str]:
        logging.info("MockSheetService: get_existing_urls called (%s)", csv_path)
        return set()

    def upsert_signal(self, signal: Dict[str, Any]) -> None:
        logging.info("MockSheetService: upsert_signal called for %s", signal.get("url"))

    def update_signal_by_url(self, req: UpdateSignalRequest) -> Dict[str, str]:
        logging.info("MockSheetService: update_signal_by_url called for %s", req.url)
        return {"status": "success", "message": "Mock update"}

    def update_sheet_enrichment(self, url: str, analysis: str, implication: str) -> bool:
        logging.info("MockSheetService: update_sheet_enrichment called for %s", url)
        return False

    def update_local_csv(self, url: str, analysis: str, implication: str, csv_path: str = "Nesta Signal Vault - Sheet1.csv") -> bool:
        logging.info("MockSheetService: update_local_csv called for %s", url)
        return False

    def update_local_signal_by_url(self, req: UpdateSignalRequest, csv_path: str = "Nesta Signal Vault - Sheet1.csv") -> bool:
        logging.info("MockSheetService: update_local_signal_by_url called for %s", req.url)
        return False
...
class SearchService:
    ...
    async def search(
        ...
        source_types_key = tuple(source_types or [])
        return await self._cached_search(query, date_restrict, requested_results, source_types_key)
