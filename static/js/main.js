...
async function runScan() {
  ...
  try {
    ...
    if (state.currentMode === "deep_dive" || state.currentMode === "research") {
      renderResearchResult(result, dom.radarFeed);
      if (result.signals && result.signals.length) {
        state.globalSignalsArray = result.signals;
      }
    } else {
    ...
    <div class="bg-nesta-purple text-white p-6">
      <div class="flex justify-between items-start">
        <div>
          ...
          <h2 class="text-3xl font-display font-bold mb-2">${escapeHtml(researchData.Title || researchData.title || "Untitled Research")}</h2>
          <p class="text-white/90 text-lg italic">"${escapeHtml(researchData.Hook || researchData.hook || "")}"</p>
        </div>
        <div class="flex flex-col items-center bg-white/10 rounded-lg p-3 backdrop-blur-sm">
          <span class="text-3xl font-bold">${researchData.Score || researchData.score || 0}%</span>
    ...
    <div class="p-8 space-y-8">
      <section>
        ...
        <div class="prose max-w-none text-gray-600 leading-relaxed">
          ${typeof marked !== 'undefined' ? marked.parse(researchData.Analysis || researchData.Description || researchData.summary || "No analysis provided.") : escapeHtml(researchData.Analysis || researchData.Description || researchData.summary || "No analysis provided.")}
        </div>
      </section>
      ...
      <div class="flex items-center justify-between text-sm text-gray-400 border-t pt-4 mt-4">
        <div class="flex gap-4">
          <span>Target Mission: ${escapeHtml(researchData.Mission || researchData.mission || "General")}</span>
          <span>Lenses: ${escapeHtml(researchData.Lenses || "N/A")}</span>
        </div>
        <a href="${escapeAttribute(sanitizeUrl(researchData.URL || researchData.url || ""))}" target="_blank"
