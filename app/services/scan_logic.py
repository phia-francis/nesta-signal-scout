"""
Production-ready scan orchestrator with international policy support.
FIXED: Policy mode now includes global government sources, not just UK.
"""
...
# Source-specific scoring divisors
GTR_FUNDING_DIVISOR = 25_000.0
OPENALEX_CITATION_DIVISOR = 100.0
GOOGLE_BASELINE_ATTENTION = 7.0

# Cache settings
FETCH_CACHE_TTL_SECONDS = 24 * 60 * 60  # 24 hours
...
class ScanOrchestrator:
    """
    Coordinates fetching, filtering, and scoring of signals across multiple sources.
    Supports three main modes: Radar (discovery), Research (synthesis), Policy (targeted).
    """
    ...
    def cutoff_date(self) -> datetime:
        """Return datetime one year ago for radar/policy mode filtering."""
    ...
    def _clone_raw_signals(self, signals: list[RawSignal]) -> list[RawSignal]:
        """Deep copy signals to avoid cache mutation."""
    ...
    async def fetch_signals(
        ...
        *,
        ...
        """
        Fetch signals from all providers concurrently with 24h cache.
        Returns: (raw_signals, related_terms)
        """
        ...
        if cached and (time.monotonic() - cached[0]) < FETCH_CACHE_TTL_SECONDS:
            ...
            logging.info("Cache hit for topic: %s", clean_topic)
            return self._clone_raw_signals(cached_signals), list(cached_terms)

        # Generate related terms for breadth
        try:
        ...
        gtr_result, openalex_result, google_year_result, google_month_result = await asyncio.gather(
            ...
            self.search_service.search(
                clean_topic, num=SCAN_RESULT_LIMIT, freshness="year", friction_mode=friction_mode
            ),
            self.search_service.search(
                clean_topic, num=SCAN_RESULT_LIMIT, freshness="month", friction_mode=friction_mode
            ),
            return_exceptions=True,
        )

        # Normalise results from each source
        ...
        raw_signals.extend(
            self._normalise_google(
                google_year_result, mission=mission, source_label="Google Search", is_novel=False
            )
        )
        raw_signals.extend(
            self._normalise_google(
                google_month_result, mission=mission, source_label="Google Search", is_novel=True
            )
        )

        # Update cache
        self._fetch_cache[cache_key] = (time.monotonic(), self._clone_raw_signals(raw_signals), list(related_terms))
        logging.info("Fetched %d signals for topic: %s", len(raw_signals), clean_topic)
    ...
    def process_signals(
        ...
        *,
        ...
        """
        Apply recency filtering, calculate scores, and yield deduplicated SignalCards.
        """
        ...
        for raw_signal in raw_signals:
            ...
            final = (activity * ACTIVITY_WEIGHT) + (attention * ATTENTION_WEIGHT) + (recency * RECENCY_WEIGHT)
    ...
    async def fetch_policy_scan(self, topic: str) -> list[SignalCard]:
        """
        FIXED: Policy-focused scan with INTERNATIONAL government sources.
        Previously limited to site:gov.uk, now includes global government domains.
        """
        if not topic.strip():
            raise ValueError("Policy topic is required.")

        # INTERNATIONAL POLICY QUERY:
        # Includes: .gov (US), .gov.uk (UK), .gov.au (Australia), .gov.ca (Canada),
        # .int (International orgs), .org (Multilateral bodies), and policy PDFs
        query = f'{topic} (site:.gov OR site:.gov.uk OR site:.gov.au OR site:.gov.ca OR site:.int OR site:.org OR filetype:pdf) "policy"'

        try:
            results = await self.search_service.search(query, num=SCAN_RESULT_LIMIT, freshness="year")
        except (ServiceError, Exception) as e:
            logging.error("Policy search failed for topic '%s': %s", topic, e)
            results = []
    ...
    async def fetch_research_deep_dive(self, query: str) -> list[SignalCard]:
        """
        OpenAlex-driven research cards over 5-year horizon (for comparison with synthesis).
        This provides individual cards, not synthesis. Use Research route for synthesis.
        """
        if not query.strip():
        ...
        try:
            ...
            logging.error("OpenAlex search failed for query '%s': %s", query, e)
            works = []
        ...
        return list(
            self.process_signals(
                raw_signals, mission="Research", related_terms=[], override_cutoff_date=override_cutoff
            )
        )

    async def fetch_intelligence_brief(self, topic: str) -> list[SignalCard]:
        """
        Fast high-level brief combining GtR and OpenAlex (1-year horizon).
        """
        if not topic.strip():
    ...
    # -------------------------------------------------------------------------
    # Normalisation methods (convert source-specific formats to RawSignal)
    # -------------------------------------------------------------------------

    def _normalise_gtr(self, result: Any, *, mission: str) -> list[RawSignal]:
        """Normalise UKRI Gateway to Research project data."""
    ...
    def _normalise_openalex(self, result: Any, *, mission: str) -> list[RawSignal]:
        """Normalise OpenAlex research publication data."""
        if isinstance(result, Exception) or not isinstance(result, list):
            return []
    ...
    def _normalise_google(
        self, result: Any, *, mission: str, source_label: str, is_novel: bool
    ) -> list[RawSignal]:
        """Normalise Google Custom Search results."""
        if isinstance(result, Exception) or not isinstance(result, list):
            return []
            ...
            signals.append(
                RawSignal(
                    ...
                    metadata={
                        "trust": trust,
                        "rank": rank,
                        "freshness": "month" if is_novel else "year",
                    },
                    is_novel=is_novel,
                )
            )
    ...
    def _calculate_activity(self, raw_signal: RawSignal) -> float:
        """Calculate activity score based on source type and metadata."""
        if raw_signal.source == "UKRI GtR":
            return min(ACTIVITY_CAP, raw_signal.raw_score / GTR_FUNDING_DIVISOR)
        if raw_signal.source == "OpenAlex":
            return 0.0  # OpenAlex doesn't indicate activity, only attention via citations
    ...
    def _calculate_attention(self, raw_signal: RawSignal) -> float:
        """Calculate attention score based on source type and metadata."""
        if raw_signal.source == "UKRI GtR":
            return 0.0  # GtR doesn't indicate public attention, only funding activity
        if raw_signal.source == "OpenAlex":
    ...
    def _normalise_url(self, url: str) -> str:
        """Normalise URL for deduplication (remove protocol, www, trailing slash)."""
    ...
    def _deduplicate_signals(self, signals: list[SignalCard]) -> list[SignalCard]:
        """
        Remove duplicate signals by URL and fuzzy title similarity.
        TODO: For large datasets, consider LSH or SimHash for O(n) complexity.
        """
        ...
        for signal in signals:
            ...
            # Check fuzzy title duplication
            is_fuzzy_duplicate = False
            for existing in kept:
                similarity = SequenceMatcher(
                    None, signal.title.strip().lower(), existing.title.strip().lower()
                ).ratio()
    ...
    def _build_sparkline(scored: ScoredSignal) -> list[int]:
        """Generate sparkline data for UI visualisation."""
    ...
    def _to_signal_card(scored: ScoredSignal, *, related_terms: list[str]) -> SignalCard:
        """Convert ScoredSignal to SignalCard for API response."""
        return SignalCard(
            title=scored.title,
            url=scored.url,
